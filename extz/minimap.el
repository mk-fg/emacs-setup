...
(defcustom minimap-display-semantic-overlays nil
...
(defcustom minimap-enlarge-certain-faces 'always
...
(defun minimap-get-window ()
  "Get current minimap window."
  (when (get-buffer minimap-buffer-name)
    (get-buffer-window minimap-buffer-name t)))

(defun minimap-kill-buffer ()
...
(defun mmap ()
	...
	(interactive)
	(if (not (and fg-mmap-allow minimap-mode))
		(progn
			(setq fg-mmap-allow t)
			(minimap-mode))
		(minimap-kill)
		(minimap-mode 0)))
...
(define-minor-mode minimap-mode
  ...
  :group 'minimap
  :lighter " MMap"
  (if (and minimap-mode fg-mmap-allow)
      (progn
...
(defun minimap-move-overlay-mouse (start-event)
  ...
  (when (get-buffer-window (buffer-base-buffer (current-buffer)) t)
	...
	(while (and
      ...
      (select-window (get-buffer-window (buffer-base-buffer)) t)
      (minimap-update)
      (when (and pcselmode (fboundp 'pc-selection-mode))
	(pc-selection-mode 1)))))
...
(defun minimap-set-overlay (pt)
  ...
  (let* ((ovstartline (line-number-at-pos minimap-start))
    ...
    (with-selected-window (get-buffer-window (buffer-base-buffer) t)
      (goto-char pt)
...
(defvar fg-mmap-enlarge-k 0.8
	"Default font height multiplier for enlarged faces.")

(defun minimap-enlarge-faces ()
	"Apply default font to all faces in `minimap-normal-height-faces'."
	(let ((pos (next-single-property-change (point-min) 'face)) next ov face)
		(while pos
			(setq face (get-text-property pos 'face))
			(when (and face (member face minimap-normal-height-faces))
				(with-current-buffer minimap-buffer-name
					(setq ov (make-overlay pos
						(setq pos (next-single-property-change pos 'face))))
					(overlay-put ov 'face `(:height
						,(floor (* fg-mmap-enlarge-k (face-attribute 'default :height)))))
